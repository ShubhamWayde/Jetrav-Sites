...
const AIR_COLUMNS: Column<LeadResponse>[] = [
  {
    key: "customerName",
    header: "Name",
    render: (t) => <span className={styles.name}>{t.customerName}</span>,
  },
  {
    key: "mobileNumber",
    header: "Mobile no.",
    render: (t) => t.mobileNumber || "—",
  },
  { key: "source", header: "Origin", render: (t) => det(t, "source") },
  {
    key: "destination",
    header: "Destination",
    render: (t) => det(t, "destination"),
  },
  {
    key: "departure",
    header: "Departure",
    render: (t) => fmt(det(t, "departure")),
  },
...
export default function DashboardPage() {
  ...
  const [hotelDateFilter, setHotelDateFilter] =
    useState<TripDateFilter>("today");
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get<DashboardResponse>(
        `${ADMIN_API.DASHBOARD}?${params.toString()}`,
      );
  ...
  const airTrips = applyTripFilter(
    applyDateFilter(rawAir, "departure", airDateFilter),
    airTripFilter,
  );
  const hotelTrips = applyTripFilter(
    applyDateFilter(rawHotel, "checkIn", hotelDateFilter),
    hotelTripFilter,
  );
      ...
      {loading ? (
        <div className={styles.statsRow}>
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className={`${styles.statCard} ${styles.statCardSkeleton}`}
            />
          ))}
        </div>
      ) : error ? (
        <div className={styles.errorBanner}>
          <span>{error}</span>
          <Button title="Retry" variant="secondary" onClick={fetchDashboard}>
            Retry
          </Button>
        </div>
      ) : (
        ...
        <div className={styles.tripHeader}>
          <div className={styles.tripTitleRow}>
            <h3 className={styles.tripTitle}>
              Upcoming Air Trips
              {!loading && (
                <span className={styles.tripCount}>{airTrips.length}</span>
              )}
            </h3>
            <Tabs
              options={TRIP_FILTER_OPTS}
              active={airTripFilter}
              onChange={setAirTripFilter}
            />
          </div>
          <Tabs
            options={TRIP_DATE_OPTS}
            active={airDateFilter}
            onChange={setAirDateFilter}
          />
        </div>
        {loading ? (
        ...
        <div className={styles.tripHeader}>
          <div className={styles.tripTitleRow}>
            <h3 className={styles.tripTitle}>
              ...
              {!loading && (
                <span className={styles.tripCount}>{hotelTrips.length}</span>
              )}
            </h3>
            <Tabs
              options={TRIP_FILTER_OPTS}
              active={hotelTripFilter}
              onChange={setHotelTripFilter}
            />
          </div>
          <Tabs
            options={TRIP_DATE_OPTS}
            active={hotelDateFilter}
            onChange={setHotelDateFilter}
          />
        </div>
        {loading ? (
          <div className={styles.tableLoading}>
