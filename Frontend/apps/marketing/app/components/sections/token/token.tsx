...
const CARD_DATA = [
  ...
  },
  {
    ...
    description: "Track your growing balance on your Jetrav Dashboard and redeem them seamlessly on your next flight, hotel, or visa booking.",
      ...
      <div className={clx("container")}>
        <div>
          <InfoBlock contentClassName={clx("items-center", "text-center")} icon={
            <Image src={"/img-token.png"} alt={"Token"} width={160} height={160} sizes={"100vw"} />
          } title={"A Tokenized Travel Economy."}
                     description={"We replaced hidden middleman commissions with a dynamic reward ledger.\n" +
                       "Every trip funds your next trip."} />
        </div>
        <div className={clx(styles.grid, "grid", "grid-cols-3", "md-grid-cols-2", "sm-grid-cols-1", "gap-4", "mt-12")}>
          {CARD_DATA.map((card) => (
